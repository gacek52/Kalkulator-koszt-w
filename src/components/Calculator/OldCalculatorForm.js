 ...
 *
 * STARA WERSJA (zachowana dla backward compatibility)
 * Nowa wersja: NewCalculatorForm.js (FAZA 5)
 *
 * Ten komponent pozostaje nietknięty jako fallback.
 */
export function OldCalculatorForm({ tab, tabs, tabIndex, globalSGA, calculationMeta, themeClasses, darkMode, onOpenSettings }) {
          ...
          if (FEATURE_FLAGS.USE_EXTRACTED_ITEM_CARD) {
            return (
              <ItemCard
                ...
                canDelete={tab.items.length > 1}
              />
            );
          }

          // ERROR: Feature flag disabled! This fallback code has been removed.
          // Please enable USE_EXTRACTED_ITEM_CARD in featureFlags.js
          throw new Error('[OldCalculatorForm] USE_EXTRACTED_ITEM_CARD must be enabled - fallback code removed');
